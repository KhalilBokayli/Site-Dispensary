...
function emailExistsInTable($con, $email, $table)
{
...
if (isset($_POST['docsub1']) || isset($_POST['docsub2'])) {
  $fname = $_POST['fname'];
  $lname = $_POST['lname'];
  $gender = $_POST['gender'];
  $email = $_POST['email'];
  ...
  if ($password === $cpassword) {
    $current_table = isset($_POST['docsub1']) ? 'patreg' : 'patlab';

    if (emailExistsInTable($con, $email, $current_table)) {
      echo "You are already registered in this category.";
      redirectToLogin();
    } else {
      $stmt = $con->prepare("INSERT INTO $current_table (fname, lname, gender, email, contact, password) VALUES (?, ?, ?, ?, ?, ?)");
      ...
      if ($stmt->execute()) {
        ...
        $redirect = isset($_POST['docsub1']) ? 'admin-panel.php' : 'admin-panel2.php';
