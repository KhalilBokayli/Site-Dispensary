...
if (isset($_POST['patsub'])) {
  ...
  if (mysqli_num_rows($result) == 1) {
    ...
    echo ("<script>alert('Invalid Username or Password. Try Again!'); window.location.href = 'index.html';</script>");
  }
} elseif (isset($_POST['patsub1'])) {
  $email = $_POST['email'];
  $password = $_POST['password2'];
  $query = "SELECT * FROM patlab WHERE email='$email' AND password='$password';";
  $result = mysqli_query($con, $query);
  if (mysqli_num_rows($result) == 1) {
    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
      $_SESSION['pid'] = $row['pid'];
...
<body style="padding-top:50px;">
  ...
  <!--Sweet alert js-->
  <script src="https://cdnjs.cloudflare.com/ajax/libs/limonte-sweetalesrt2/7.33.1/sweetalert2.all.js"></script>
